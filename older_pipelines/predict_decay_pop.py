"""Pipeline de predicao do modelo de popularidade com decaimento exponencial
(decayPop).

Mesma logica de predicao do POPModel (predict_pop.py): para cada
interacao de teste, ranqueia N_RECS apps dentre os que o usuario ainda
nao havia consumido. A diferenca esta na matriz de popularidade usada
para pontuar os itens -- em vez da soma cumulativa bruta desde o inicio
do historico, cada interacao passada contribui com um peso
exp(-lambda * (t - ti)), onde `t` e a data de referencia (o dia atual)
e `ti` o dia em que a interacao ocorreu. Interacoes mais antigas pesam
cada vez menos, controladas por `lambda`.

Como a matriz depende de `lambda`, ela precisa ser construida em tempo
de execucao (utils.decay_popularity_matrix.build_decay_popularity_matrix)
...
de predict_pop.py. O catalogo de itens usado como candidato e sempre o
catalogo inteiro (sem corte), como em optimize_pop.py.

LAMBDA pode ser definido por linha de comando (--lambda/-l). Sem
argumento, usa o default abaixo. Exemplos:
    python predict_decay_pop.py                # LAMBDA = 0.01 (default)
    python predict_decay_pop.py --lambda 0.05
    python predict_decay_pop.py -l 0.2
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Pipeline de predicao do modelo de popularidade com decaimento exponencial (decayPop)."
    )
    parser.add_argument(
        "--lambda",
        "-l",
...
if __name__ == "__main__":
    ...
    output_path = f"data/predictions/decay_pop_{args.lambda_}.parquet"
