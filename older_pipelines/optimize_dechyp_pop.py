"""Otimizacao do hiperparametro `lambda` do dechypPop via Optuna.

Busca o valor de `lambda` (fator de decaimento hiperbolico, 0.001 a 1)
...
dechypPop e identico ao decayPop (optimize_decay_pop.py) exceto pela
funcao de decaimento: em vez do peso exponencial exp(-lambda * (t - ti)),
cada interacao passada contribui com um peso hiperbolico
1 / (1 + lambda * (t - ti)), onde `t` e a data de referencia e `ti` o dia
em que a interacao ocorreu (ver
utils.decay_popularity_matrix.hyperbolic_decay_from_daily_counts). O
catalogo de itens candidatos e sempre o catalogo inteiro (sem corte),
como em optimize_pop.py / optimize_decay_pop.py.

Fluxo completo (dois scripts separados):
  1. `optimize_dechyp_pop.py` (este arquivo): treino -> prediz validacao,
     escolhe o melhor `lambda` por NDCG@20 no val.
  2. `predict_dechyp_pop.py --lambda <melhor>` + `evaluate_predictions.py`:
     roda a predicao de fato sobre o split de teste (unica vez, com o
     lambda ja escolhido) para obter a metrica real reportada.

Nao sobrescreve predict_dechyp_pop.py -- reaproveita sua funcao de
...
inteiro (~19M interacoes) sem repetir trabalho independente de `lambda` a
cada trial:
  ...
  2. `score_lambda(lambda_, prepared)`: roda 1x por trial. Aplica o
     decaimento hiperbolico sobre as contagens diarias ja pivotadas
     (`hyperbolic_decay_from_daily_counts`, unica parte que de fato
     depende de `lambda`) e pontua cada linha de val, reaproveitando tudo
     que `prepare()` ja calculou.

     Diferente do decayPop, o peso hiperbolico nao e telescopico e nao
     admite a recorrencia O(n_datas) usada em `decay_from_daily_counts` --
     `hyperbolic_decay_from_daily_counts` calcula a matriz via produto
     denso (n_datas x n_datas) @ (n_datas x n_itens), O(n_datas^2 *
     n_itens) por trial. Na pratica isso e tao rapido quanto (ou mais
     rapido que) a recorrencia do decayPop no dataset atual (~3.4 mil
     datas com interacao, ~10 mil itens): o produto denso e delegado ao
     BLAS, enquanto a recorrencia roda um loop Python de ~3.4 mil
     iteracoes. N_TRIALS mantido igual a optimize_decay_pop.py.
...
Busca:
  ...
  - Alguns trials iniciais fixos (via `study.enqueue_trial`), cobrindo a
    ...
    (total: N_TRIALS). Como em optimize_decay_pop.py, `lambda_` e
    continuo -- nao ha necessidade de um sampler que evite valores
    repetidos.
...
predict_dechyp_pop.py.

Uso:
    python optimize_dechyp_pop.py
    python predict_dechyp_pop.py --lambda <melhor_lambda_encontrado>
...
from utils.decay_popularity_matrix import hyperbolic_decay_from_daily_counts, pivot_daily_counts

SEED = 42
RESULTS_PATH = f"data/optuna/dechyppop/{SEED}/optuna_dechyp_pop_lambda_results.csv"
STUDY_PATH = f"data/optuna/dechyppop/{SEED}/optuna_dechyp_pop_study.pkl"
...
def prepare(df: pl.DataFrame, raw_df: pl.DataFrame) -> dict:
    ...
    assert daily_counts[DATE_COL].is_sorted(), (
        ...
        "np.searchsorted abaixo quanto a matriz de pesos em "
        "hyperbolic_decay_from_daily_counts assumem isso"
    )
...
def score_lambda(lambda_: float, prepared: dict) -> pl.DataFrame:
    """Gera as predicoes do dechypPop para um `lambda` especifico,
    reaproveitando tudo que ja foi pre-calculado em `prepare`. Mesma
    logica de ranking/desempate de predict_dechyp_pop.py (`_rank_top_n`)."""
    ...
    matrix = hyperbolic_decay_from_daily_counts(daily_counts, lambda_, DATE_COL)
